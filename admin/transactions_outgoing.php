...
// --- Generate Transaction ID for Outgoing ---
function generateTransactionID($pdo, $prefix = 'TK-') {
    $stmt = $pdo->query("SELECT MAX(SUBSTRING(transaction_code, 4)) FROM transactions WHERE transaction_code LIKE 'TK-%'");
    $lastNum = (int)$stmt->fetchColumn();
    $newNum = $lastNum + 1;
    return $prefix . str_pad($newNum, 5, '0', STR_PAD_LEFT);
}

$transaction_id = generateTransactionID($pdo, 'TK-');
...
// --- Handle Form Submission (Add Outgoing Transaction) ---
if (isset($_POST['add_outgoing_transaction'])) {
    $input_transaction_id = trim($_POST['transaction_id']);
    $transaction_date = trim($_POST['transaction_date']);
    $item_id = (int)$_POST['item_id'];
    $quantity_out = (int)$_POST['quantity_out']; // Jumlah keluar

    if (empty($input_transaction_id) || empty($transaction_date) || $item_id <= 0 || $quantity_out <= 0) {
        ...
        try {
            ...
            // 2. Validasi stok: mencegah stok negatif secara default
            if ($current_item_stock < $quantity_out) {
                $error_message = 'Jumlah keluar (' . $quantity_out . ') melebihi stok yang tersedia (' . $current_item_stock . '). Transaksi tidak dapat diproses.';
            } else {
                // 3. Insert transaksi barang keluar
                $stmt_insert_transaction = $pdo->prepare("INSERT INTO transactions (transaction_code, transaction_type, item_id, quantity, transaction_date) VALUES (?, 'outgoing', ?, ?, ?)");
                $stmt_insert_transaction->execute([$input_transaction_id, $item_id, $quantity_out, $transaction_date]);

                // 4. Update stok barang di tabel items
                $new_stock = $current_item_stock - $quantity_out;
                $stmt_update_item_stock = $pdo->prepare("UPDATE items SET quantity = ? WHERE id = ?");
                $stmt_update_item_stock->execute([$new_stock, $item_id]);

                $success_message = 'Data barang keluar berhasil disimpan dan stok diperbarui.';
                // Redirect untuk membersihkan POST data
                redirect(BASE_URL . '/admin/transactions_outgoing.php?msg=added'); // Redirect ke halaman ini sendiri
            }
...
$page_title = 'Entri Data Barang Keluar';
$quantity_label = 'Jumlah Keluar';
...
<body>
                    ...
                    <div class="card-body">
                        <form method="POST" action="">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <label for="quantity_out" class="form-label"><?= $quantity_label ?> <span
                                            class="text-danger">*</span></label>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <label for="total_stock" class="form-label">Sisa Stok <span
                                            class="text-danger">*</span></label>
                            ...
                            <div class="d-flex justify-content-end mt-4">
                                <button type="submit" name="add_outgoing_transaction"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const quantityOutInput = document.getElementById('quantity_out'); // Jumlah Keluar
        ...
        function updateTotalStock() {
            const quantityOut = parseInt(quantityOutInput.value) || 0;
            let totalStock = selectedItemCurrentStock - quantityOut;
            totalStockInput.value = totalStock;

            // Visual feedback if stock goes negative for outgoing
            if (totalStock < 0) {
                totalStockInput.classList.add('is-invalid'); // Add Bootstrap invalid class
                quantityOutInput.classList.add('is-invalid');
            } else {
                totalStockInput.classList.remove('is-invalid');
                quantityOutInput.classList.remove('is-invalid');
            }
        }
        ...
        itemIdSelect.addEventListener('change', function() {
            ...
            if (itemId) {
                ...
                updateTotalStock(); // Calculate total stock based on new current stock
            } else {
        ...
        // Event listener for Jumlah Keluar input
        quantityOutInput.addEventListener('input', updateTotalStock);
